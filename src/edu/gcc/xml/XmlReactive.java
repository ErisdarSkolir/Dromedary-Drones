...
/**
 * 
 * This class represents a query to an XML file that can change over time.
 * Whenever a new element is inserted, updated, or removed from the XML file,
 * the listeners on any XmlReactive that matches the new element will be called
 * with the new object or list.
 * 
...
public class XmlReactive<O extends Observable> {
	...
	public XmlReactive(final O observable, final String query) {
		...
		try {
			...
			throw new XmlReadException(
					String.format(
						"Could not compile xPath for reative %s",
						xPath
					),
					e
			);
	...
	public boolean matches(final String element) {
		try {
			...
			throw new XmlReadException(
					String.format(
						"Could not evaulate element for reactive %s",
						element
					),
					e
			);
